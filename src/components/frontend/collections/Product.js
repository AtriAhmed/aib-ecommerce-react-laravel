...
export default function Product({product,discount,isNew}) {
  ...
  return (
    ...
    <Link className="text-decoration-none card-img-top d-block overflow-hidden" to={`/collections/${product.category.slug}/${product.slug}`}><img className='' height={200+"px"} src={`http://localhost:8000/${product.image}`} alt="Product"/></Link>
